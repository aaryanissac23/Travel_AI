...
def validate_inputs(destination: str, budget: float):
    if not destination or not destination.strip():
        raise ValueError("Validation Error: Destination cannot be empty.")
    if budget < 0:
...
async def fetch_flights(destination: str, iteration: int) -> List[dict]:
    print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}]   -> [fetch_flights] Fetching flights...")
    await asyncio.sleep(1.0)
    discount = 0.8 ** iteration
    
    flights = []
    for f in load_travel_data().get("flights", []):
        price = f["price"] * discount
        if price < 0:
            raise ValueError(f"Validation Error: Flight price {price} is negative.")
        flights.append({"name": f["name"], "price": round(price, 2)})
    return flights

async def fetch_hotels(destination: str, iteration: int) -> List[dict]:
    ...
    for h in load_travel_data().get("hotels", []):
        price = h["price"] * discount
        if price < 0:
            raise ValueError(f"Validation Error: Hotel price {price} is negative.")
        hotels.append({"name": h["name"], "price": round(price, 2)})
    return hotels

#Defining the nodes of the Graph

async def gather_data_node(state: TravelState) -> dict:
    validate_inputs(state["destination"], state["budget"])
    
    print(f"\n[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [gather_data_node] Fetching parallel data (Iteration {state['iteration_count']})...")
    start = time.time()
    
    # Gathering flights and hotels data asynchronously
    flights, hotels = await asyncio.gather(
        fetch_flights(state["destination"], state["iteration_count"]),
        fetch_hotels(state["destination"], state["iteration_count"])
    )
    
    print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [gather_data_node] Parallel fetch took {time.time() - start:.3f} seconds.")
    return {"flights": flights, "hotels": hotels}

def select_itinerary_node(state: TravelState) -> dict:
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [select_itinerary_node] Selected cheapest options:")
...
async def run_planner(destination: str, budget: float):
    ...
    print("\n" + "="*40 + f"\nPlanning trip to {destination} | Budget: ${budget:.2f}\n" + "="*40)
...
async def main():
    ...
    while True:
        print("\n=== Travel Planner Menu ===")
        print("1. Run Success Demo (Tokyo, Budget $800)")
        print("2. Run Self-Correction Demo (Tokyo, Budget $150)")
        print("3. Run Failure Demo (Tokyo, Budget $80)")
        print("4. Custom Input")
        print("5. Exit")
        
        choice = input("Choice (1-5): ").strip()
        if choice == "1":
            await run_planner("Tokyo", 800.0)
        elif choice == "2":
            await run_planner("Tokyo", 150.0)
        elif choice == "3":
            try:
                await run_planner("Tokyo", 80.0)
            except RuntimeError as e:
                print(f"\n[Expected Exception Raised]:\n{e}")
        elif choice == "4":
            dest = input("Enter destination: ").strip()
            budget = float(input("Enter budget: ").strip())
            await run_planner(dest, budget)
        elif choice == "5":
            break
